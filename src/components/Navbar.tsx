...
const navShellClassName = `
  ...
  border-b border-[#e4e7ed] px-4 outline-none lg:px-0
`;
  ...
  function renderHeroNavbar() {
    ...
    return (
      <nav className={`absolute ${navShellClassName}`} aria-hidden={isPastHero}>
        <NavBackground />
        <div className="relative flex w-full items-center justify-between text-text-primary lg:mx-auto lg:max-w-[90dvw]">
          <NavLogo onHomeClicked={handleHomeClicked} />
        </div>
      </nav>
    );
  }
  ...
  function renderInstallButton() {
    ...
    return (
      <button
        ...
        className="rounded-lg bg-accent px-4 py-2 text-sm whitespace-nowrap text-white hover:bg-accent-hover lg:text-base"
      >
